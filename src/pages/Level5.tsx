  ...
  const handleItemReorder = (currentPos:number, newPos:number) => {
    ...
    return (
        <div className="flex flex-col mt-24 justify-center items-center text-4xl">
            ...
            {arraysEqual(array, [2, 1, 4, 3, 5, 8, 6]) && arraysEqual(array, [4,3]) && <p className="text-sm">Start with the first number as the pivot, and compare it with the numbers to the right. If a number is greater than or equal to the pivot, smaller, make it the new pivot. Once it is partitioned, swap!</p>}
            {arraysEqual(array, [4, 1, 2, 3, 5, 8, 6]) && arraysEqual(array, [4,3]) && <p className="text-sm">Great job! Now the pivot will be 4.</p>}
            {arraysEqual(array, [5, 1, 2, 3, 4, 8, 6]) && arraysEqual(array, [4,3]) && <p className="text-sm">Great job! Now the pivot will be 5.</p>}
            {arraysEqual(array, [8, 1, 2, 3, 4, 5, 6]) && arraysEqual(array, [4,3]) && <p className="text-sm">Great job! Now the pivot will be 8.</p>}
            {arraysEqual(array, [6, 1, 2, 3, 4, 5, 8]) && arraysEqual(array, [4,3]) && <p className="text-sm">Great job! Now the pivot will be 6.</p>}
            {arraysEqual(array, [6, 1, 2, 3, 4, 5, 8]) && arraysEqual(array, [4,3]) && <p className="text-sm">Great job! Now the pivot will be 6. Insert it in the right spot!</p>}
            {arraysEqual(array, [1, 2, 3, 4, 5, 6, 8]) && arraysEqual(array, [4,3]) && <p className="text-sm">Congratulations! You just learned 5 new sorting algorithms!</p>}
